...
public class C11_equals_equalsIgnoreCase {
    public static void main(String[] args) {
        ...
        String s1 = "clarusway"; // s1 String pool (string havuzundan aynı referans:adrese sahip variable -> ön cep (stack memory)
        String s2 = "clarusway"; // s2 String pool (string havuzundan aynı referans:adrese sahip variable -> ön cep (stack memory)
        String s3 =new String("clarusway"); // String data type unique adres(referans) sahip obj. -> cüzdan (heap memory)
        String s4 =new String("clarusway"); // String data type unique adres(referans) sahip obj. -> cüzdan (heap memory)

        System.out.println("(s1 == s2) = " + (s1 == s2)); // true
        System.out.println("s1.equals(s2) = " + s1.equals(s2)); // true
        System.out.println("(s3 == s4) = " + (s3 == s4)); // false -> içerik aynı fakat new'den dolayı farklı obj dolayısıyla farklı adres
        ...
        /**
String s1 = "text"; kullanımı, eğer varsa, string sabit havuzunda (string constant pool)
bir örneği yeniden kullanır.Yoksa havuz içerisinde yeniden alan oluşturur.
Bu durum Java'ya özgü Java Sanal Makinesi (JVM) tarafından gerçekleştirilen bir durumdur.
s1 ve s2 String constant pool içerisinde aynı referans değere (reference) sahiptir.
s3 ve s4 ise farklı nesneleri ifade eder.s1 == s2 , s1 ve s2 'nin hafızadaki
adres değerlerini karşılaştırır ve  String constant pool üzerinde aynı adresi gösterirler.
Böylece ekrana TRUE yazdırır.s1.equals(s2) ifadesi ise, nesneler içerisindeki değerleri
(clarusway) karşılaştırdığı için ekrana TRUE yazdırır.
